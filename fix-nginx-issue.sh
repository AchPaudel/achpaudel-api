...
echo "🔧 Creating nginx troubleshooting script..."

# Create a comprehensive nginx debug and fix script
cat > nginx-fix.sh << 'NGINX_EOF'
#!/bin/bash
...
echo "scp nginx-fix.sh ubuntu@api.achpaudel.dev:/tmp/"
echo "ssh ubuntu@api.achpaudel.dev 'chmod +x /tmp/nginx-fix.sh && sudo /tmp/nginx-fix.sh'"
